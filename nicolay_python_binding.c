...
static PyObject *NicolayError_Set(NicolayError error_code)
{
	...
	if (error_code >= NICOLAY_ERR_EXCEPTION_BASE) {
		...
		exception_type =
			PyExc_IOError; // More specific than RuntimeError
		int device_exception_code =
			NICOLAY_ERR_EXCEPTION_BASE - error_code;
		...
		case NICOLAY_EXC_WRONG_NUM_DATA:
			error_msg =
				"Nicolay device error: Wrong number of data bytes.";
			break;
		// Add more cases for specific NICOLAY_EXC_* codes
		default:
			PyErr_Format(PyExc_IOError,
				     "Nicolay device returned exception "
				     "code: %d",
		...
		case NICOLAY_ERR_INVALID_RESPONSE:
			...
			error_msg =
				"Invalid device response (e.g., wrong address).";
			break;
...
typedef struct {
	PyObject_HEAD NicolaySensor sensor; // The actual C struct instance
			      ...
			      PyObject *kwds)
{
	...
	if (slave_address < 0 || slave_address > 255) {
		PyErr_SetString(PyExc_ValueError,
				"Slave address must be between 0 and 255.");
		return -1;
	}

	// Call your C library's init function
	NicolayError err =
		nicolay_init(&self->sensor, port_name, (uint8_t)slave_address);
	...
	self->sensor.slave_address =
		(uint8_t)slave_address; // Store actual address
	...
	self->sensor.current_baud_rate =
		115200; // Placeholder, adjust as needed

	return 0; // Success
}

// Method for nicolay_close()
static PyObject *NicolaySensor_close(NicolaySensorObject *self,
...
static PyObject *NicolaySensor_get_sw_version(NicolaySensorObject *self,
					      PyObject *Py_UNUSED(ignored))
{
	...
	return Py_BuildValue("{s:c, s:i, s:i}", "index_ascii",
			     sw_version.index_ascii, "version_minor",
			     sw_version.version_minor, "version_major",
			     sw_version.version_major);
}

// Method for nicolay_get_hw_version()
static PyObject *NicolaySensor_get_hw_version(NicolaySensorObject *self,
					      PyObject *Py_UNUSED(ignored))
{
	...
	return Py_BuildValue("{s:i, s:i}", "version_minor",
			     hw_version.version_minor, "version_major",
			     hw_version.version_major);
}

// Method for nicolay_test_command()
static PyObject *NicolaySensor_test_command(NicolaySensorObject *self,
					    PyObject *Py_UNUSED(ignored))
{
	uint8_t byte1, byte2;
...
static PyObject *NicolaySensor_get_pressure(NicolaySensorObject *self,
					    PyObject *Py_UNUSED(ignored))
{
	int16_t pressure_counts;
	NicolayError err =
		nicolay_get_pressure(&self->sensor, &pressure_counts);
	...
	return Py_BuildValue("h",
			     pressure_counts); // 'h' for short int (int16_t)
}

// Method for nicolay_get_flow_and_pressure()
static PyObject *
NicolaySensor_get_flow_and_pressure(NicolaySensorObject *self,
				    PyObject *Py_UNUSED(ignored))
{
	int32_t flow_mSlm;
	int16_t raw_pressure_counts;
	NicolayError err = nicolay_get_flow_and_pressure(
		&self->sensor, &flow_mSlm, &raw_pressure_counts);
	...
	return Py_BuildValue(
		"(ii)", flow_mSlm,
		raw_pressure_counts); // 'i' for int32_t, 'i' for int16_t
}

// Method for nicolay_get_flow_measurement()
static PyObject *NicolaySensor_get_flow(NicolaySensorObject *self,
					PyObject *Py_UNUSED(ignored))
{
	int32_t flow_mSlm;
	NicolayError err =
		nicolay_get_flow_measurement(&self->sensor, &flow_mSlm);

	if (err != NICOLAY_OK) {
		return NicolayError_Set(err);
	}

	return Py_BuildValue("i", flow_mSlm);
}

// Method for nicolay_set_uart_baud()
// This method takes a baud_code and returns the response_baud_code
static PyObject *NicolaySensor_set_uart_baud(NicolaySensorObject *self,
					     PyObject *args)
{
	uint8_t baud_code;
	if (!PyArg_ParseTuple(args, "B",
			      &baud_code)) { // "B" for unsigned char (uint8_t)
		return NULL;
	}

	uint8_t response_baud_code;
	NicolayError err = nicolay_set_uart_baud(&self->sensor, baud_code,
						 &response_baud_code);
...
static PyMethodDef NicolaySensor_methods[] = {
	...
	{ "get_sw_version", (PyCFunction)NicolaySensor_get_sw_version,
	  METH_NOARGS, "Retrieves the sensor's software version." },
	{ "get_hw_version", (PyCFunction)NicolaySensor_get_hw_version,
	  METH_NOARGS, "Retrieves the sensor's hardware version." },
	...
	{ "get_flow_and_pressure",
	  (PyCFunction)NicolaySensor_get_flow_and_pressure, METH_NOARGS,
	  "Retrieves both flow (mSlm^2) and raw pressure counts." },
	{ "get_flow", (PyCFunction)NicolaySensor_get_flow, METH_NOARGS,
	  "Retrieves flow (mSlm^2)." },
	{ "set_uart_baud", (PyCFunction)NicolaySensor_set_uart_baud,
	  METH_VARARGS,
...
static PyMemberDef NicolaySensor_members[] = {
	{ "slave_address", T_UBYTE,
	  offsetof(NicolaySensorObject, sensor.slave_address), READONLY,
	...
	{ "current_baud_rate", T_INT,
	  offsetof(NicolaySensorObject, sensor.current_baud_rate), READONLY,
...
static PyTypeObject NicolaySensorType = {
	PyVarObject_HEAD_INIT(NULL, 0).tp_name =
		"nicolay_flow_meter_connector.NicolaySensor", // ModuleName.ClassName
	...
	.tp_dealloc =
		(destructor)NicolaySensor_dealloc, // Our custom deallocator
...
PyMODINIT_FUNC PyInit_nicolay_flow_meter_connector(void)
{
	...
	if (PyModule_AddObject(m, "NicolaySensor",
			       (PyObject *)&NicolaySensorType) < 0) {
